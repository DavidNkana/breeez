...
export default async function AdminOrdersPage() {
  ...
  try {
    ...
    if (error) {
      ...
      logError({ message: `[admin/orders] query failed: ${queryError}`, severity: 'error' });
    } else {
      orders = (data ?? []) as any[];
    }
  } catch (err: any) {
    queryError = err?.message ?? 'Failed to load orders';
    logError({ message: `[admin/orders] thrown: ${queryError}`, severity: 'error' });
  }
    ...
    <>
      <main className="mx-auto max-w-6xl min-w-0 overflow-x-hidden px-4 py-10 pb-20 safe-bottom">
        <div className="flex items-center justify-between gap-2 flex-wrap">
          ...
          <Link href="/admin/customers" className="text-sm text-brand-700 dark:text-brand-300 underline hover:text-brand-900 dark:hover:text-white">
            Customer list →
          </Link>
        </div>
        ...
        {queryError && (
          <div className="mt-6 rounded-lg border border-red-200 bg-red-50 p-4 dark:border-red-800 dark:bg-red-950/40">
            <p className="font-medium text-red-900 dark:text-red-300">Failed to load orders</p>
            <p className="mt-1 text-sm text-red-700 dark:text-red-400">{queryError}</p>
            <p className="mt-1 text-xs text-red-600 dark:text-red-500">
              Check the Vercel deployment logs for the full stack trace.
            </p>
          </div>
        )}

        {!queryError && orders.length === 0 && (
          <div className="mt-6 rounded-lg border border-brand-200 bg-white p-8 text-center text-sm text-brand-500 dark:border-brand-700 dark:bg-brand-900 dark:text-brand-400">
            No orders yet.
          </div>
        )}
              ...
              <tbody className="divide-y divide-brand-100 dark:divide-brand-700">
                {orders.map((o) => (
                  <tr key={o.id}>
                    ...
                    <td className="px-4 py-3 font-medium text-brand-950 dark:text-white">{formatRand(o.total_cents)}</td>
