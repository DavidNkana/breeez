-- The return value is the exact order_item id used as the compensation token.
-- Reject stale carts that reference an inactive variant at the same locked,
-- authoritative point where stock is checked and decremented.
...
begin
  select stock, coalesce(name, ''), is_active
    into v_stock, v_variant_name, v_is_active
  ...
  if not v_is_active then
    raise exception 'Variant % is unavailable', p_variant_id using errcode = 'P0001';
  end if;
