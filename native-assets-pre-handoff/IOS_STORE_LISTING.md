# iOS App Store Listing Text — copy-paste into App Store Connect

## App name (max 30 chars)
```
Breeez
```

## Subtitle (max 30 chars)
```
Home, Kitchen & Lifestyle
```

## Promotional text (max 170 chars, can be updated without review)
```
South Africa's curated marketplace for everyday essentials. Browse 10 categories, nationwide delivery, secure local checkout.
```

## Description (max 4000 chars)
```
Welcome to Breeez — South Africa's curated marketplace for the everyday things that make a house feel like home.

Whether you're refreshing your kitchen, restocking bathroom essentials, updating the kids' back-to-school gear, or adding the perfect accent to your living room, Breeez brings together 10 carefully chosen categories:
- Apparel
- Bags
- Bathroom
- Bedroom
- Curtains & Blinds
- Home Décor
- Kitchen
- Shoes
- Back-to-School
- Everyday Essentials

WHY BREEZ?
• Real South African essentials, not overseas noise — products chosen for local taste and local prices.
• Nationwide delivery: Pargo pickup, The Courier Guy, or Dawn Wing same-day metro.
• Pay how South Africans pay: PayFast, Yoco, Ozow, instant EFT, or card.
• Trusted SA couriers: Pargo (pickup lockers), The Courier Guy, Dawn Wing — or collect in store.
• 7-day returns on unused items (customer pays return shipping).
• Your account, address book, and wishlist sync across web and mobile.

BROWSE BY CATEGORY
Filter by price, colour, size. Sort by what's new, by what's hot, or by best price. Tap a product to see detailed photos, descriptions, and customer reviews. Save the ones you love to your wishlist.

FAST, SECURE CHECKOUT
Guest checkout or signed-in account. Save up to 5 addresses. Track your order from confirmation through delivery. Get real-time updates via email and push notifications.

YOUR ACCOUNT
• Order history and receipts
• Wishlist (synced across devices)
• Saved addresses
• Profile settings, including display name
• Returns portal: request a return in under 2 minutes from your phone
• POPIA-compliant — we never sell your data

FOR THE BUSINESS CUSTOMER
Breeez invoices for VAT-registered buyers. Email accounts@breeez.shop for bulk orders.

BUILT FOR SA
• All prices in South African Rand (ZAR). No currency surprises at checkout.
• Real SA mobile numbers supported for SMS notifications.
• Local customer support: support@breeez.shop

Made with care in Cape Town.

© 2026 Breeez (Pty) Ltd.
```

## Keywords (max 100 chars, comma-separated, no spaces)
```
shopping,home,kitchen,south africa,apparel,lifestyle,delivery,payfast,couriers,essentials
```
(Exactly 87 characters — fits.)

## What's new (max 4000 chars)
```
Welcome to Breeez — your one-stop shop for kitchen, home & lifestyle essentials.

What's inside:
• Browse 10 categories: apparel, bags, bathroom, bedroom, curtains & more
• Save items to your wishlist
• Fast checkout with PayFast, Yoco or Ozow
• SA couriers: Pargo, The Courier Guy, Dawn Wing
• Nationwide delivery via Pargo, The Courier Guy, or Dawn Wing
• 7-day returns, request right from your phone
• Account, addresses, and order history synced across web + mobile

Thanks for shopping with us.
```

## Support, marketing, privacy URLs

| URL | Use |
|---|---|
| https://breeez-lyart.vercel.app/contact | Support URL (REQUIRED) |
| https://breeez-lyart.vercel.app/ | Marketing site |
| https://breeez-lyart.vercel.app/legal/privacy | Privacy policy URL (REQUIRED) |

## Category + age rating

- Primary category: **Shopping**
- Secondary: **Lifestyle**
- Age rating: 4+ (no objectionable content, no gambling, no in-app purchases if you remove IAPs)

## Privacy + Data collection (App Privacy section)

| Data | Linked to user | Used for tracking | Collection purpose |
|---|---|---|---|
| Email address | Yes | No | Account creation |
| Name | Yes | No | Profile + shipping |
| Postal address | Yes | No | Shipping |
| Phone number | Yes | No | Shipping + support |

- **Payment info**: NOT collected by us. PayFast handles it per their policy.
- **Analytics**: None at launch.
- **Third-party SDKs**: Supabase (auth + DB), Resend (email), PayFast/Yoco/Ozow (payments).
