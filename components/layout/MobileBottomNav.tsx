  ...
  return <nav aria-label="Primary" className="fixed inset-x-0 bottom-0 z-40 border-t border-brand-200 dark:border-brand-800 bg-white/95 dark:bg-brand-950/95 backdrop-blur supports-[backdrop-filter]:bg-white/80 dark:supports-[backdrop-filter]:bg-brand-950/80 md:hidden safe-bottom">
    <ul className="mx-auto grid max-w-md grid-cols-5">
      {tabs.map((tab) => { const active = tab.href === '/' ? pathname === '/' : pathname.startsWith(tab.href); return <li key={tab.href}>
        <Link href={tab.href} className={`relative flex min-h-16 flex-col items-center justify-center gap-0.5 text-[11px] ${active ? 'font-semibold text-accent-700 dark:text-accent-600' : 'text-brand-600 dark:text-brand-300'}`}>
          <span className="relative text-xl leading-5" aria-hidden="true">{tab.icon}
            {tab.label === 'Wishlist' && mounted && wishlistCount > 0 && <span className="absolute -right-2 -top-1 inline-flex h-4 min-w-4 items-center justify-center rounded-full bg-accent-700 px-1 text-[10px] font-normal text-white">{wishlistCount}</span>}
            {tab.label === 'Cart' && mounted && count > 0 && <span className="absolute -right-2 -top-1 inline-flex h-4 min-w-4 items-center justify-center rounded-full bg-accent-700 px-1 text-[10px] font-normal text-white">{count}</span>}
